...
function generateAuthToken(username) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const uniqueAuthToken = yield getUniqueAuthToken();
        console.log("Generated unique auth token!");
        const authToken = uniqueAuthToken === null || uniqueAuthToken === void 0 ? void 0 : uniqueAuthToken.authToken;
        const expirationDate = (uniqueAuthToken === null || uniqueAuthToken === void 0 ? void 0 : uniqueAuthToken.expirationDate) ? new Date(uniqueAuthToken.expirationDate) : date;
        ...
        const hashedAuthToken = yield bcrypt.hash(authToken, 10);
        // Save the hashed authentication token and expiration date to the database
        yield queryDb(`UPDATE users
                       SET auth_token            = ?,
...
function getUniqueAuthToken() {
    return __awaiter(this, void 0, void 0, function* () {
        while (true) {
            ...
            const existingUser = yield queryDb(`
...
function authenticateUser(token) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const rows = yield queryDb(`SELECT auth_token, username FROM users`, []);
            console.log('All users:', rows);
            // Find a user with a matching token and extract their username.
            let username = null;
            for (const row of rows) {
                const match = yield bcrypt.compare(token, row.auth_token);
                if (match) {
...
app.post('/auth', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    console.log(token);
    const username = yield authenticateUser(token);
...
app.post('/login', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        let user = yield findUser(username);
        console.log(`User found: ${username}`);
        yield validateCredentials(user, username, password);
        console.log("Valid Credentials!");
        // Generate an authentication token and send it to the client in a cookie
        const { authToken, expirationDate } = yield generateAuthToken(username);
...
function validateCredentials(user, username, password) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!user) {
            ...
            const isMatch = yield bcrypt.compare(password, user.password); // Compare hashed password with stored password
            if (!isMatch) {
                // If passwords do not match, throw error
                yield sleep(5000); // Delay to prevent brute force attacks
...
app.get('/get/quizlet/data', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let quizlet_id = req.query.quizlet_id;
        const url = req.query.url;
        console.log(quizlet_id);
        if (!quizlet_id && url) {
            const quizlet_id_match = url.match(/quizlet\.com\/(?:[a-z]{2}\/)?(\d+)/);
            if (quizlet_id_match) {
        ...
        checkAndCreateDir();
        const terms = yield quizlet(Number(quizlet_id));
...
app.post('/post/typed', (req, res) => {
    const { def, term, randomIndex, username, quizlet_id } = req.body;
    checkAndCreateDir();
    db.serialize(() => {
        db.get('SELECT id FROM users WHERE username = ?', [username], (err, row) => {
            ...
            db.serialize(() => {
                ...
                const timeNow = getTimestamp();
                console.log(timeNow); // Output: "2023-03-29 08:15:30"
                db.run('INSERT INTO history (user_id, quizlet_id, randomIndex, def, term, created_at) VALUES (?, ?, ?, ?, ?, ?)', [id, quizlet_id, randomIndex, def, term, timeNow], (err) => {
                    if (err) {
                        console.error(err);
                        res.status(500).send('Internal Server Error');
                        return;
...
app.get('/get/history', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
...
app.get('/get/furigana', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { word } = req.query;
    // Check if the term has kanji
    const result = Kuroshiro.Util.hasKanji(word);
    if (result === true) {
        const furigana = yield kuroshiro.convert(word, {
            mode: "furigana",
            to: "hiragana"
        });
...
app.get('/leaderboard', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        if (req.query.quizlet_id === undefined) {
            const playtimeRows = yield queryDb("SELECT * FROM playtime", []);
            const userRows = yield queryDb("SELECT * FROM users", []);
            ...
            for (const entry of leaderboard) {
                entry.playtime = yield formatDuration(Number(entry.playtime)); // Convert back to number for calculations
            }
            ...
            const [{ quizlet_title }] = yield queryDb('SELECT quizlet_title FROM quizlet WHERE quizlet_id = ?', [quizlet_id]);
            const rows = yield queryDb('SELECT u.id AS user_id, u.username, COUNT(h.id) AS word_count FROM users u LEFT JOIN history h ON u.id = h.user_id AND h.quizlet_id = ? GROUP BY u.id HAVING word_count > 0 ORDER BY word_count DESC', [quizlet_id]);
            console.log(rows);
            const rankList = rows.map((row) => ({
                username: row.username,
                word_count: row.word_count,
                profile_url: `/profile?user=${row.username}`
            }));
            const labels = JSON.stringify(rows.map((row) => row.username));
            const data = JSON.stringify(rows.map((row) => row.word_count));
            const templatePath = path.join(__dirname, 'public', 'html', 'leaderboard', 'index.html');
            const templateSource = fs.readFileSync(templatePath, 'utf8');
            const template = handlebars.compile(templateSource);
            const html = template({
            ...
            res.header('Content-Type', 'text/html');
            res.send(html);
            yield logMessage('Sending Leaderboard...', 'info');
        }
    }
    catch (err) {
        console.error(err.message);
...
app.get('/profile', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const username = req.query.user !== undefined ? req.query.user : '';
    checkAndCreateDir();
    try {
        const result = yield getDataProfile(username);
        if (result.length !== 0) {
            const count_per_day = yield getWordCountPerDay(username);
            ...
            const playtimeMS = yield getPlaytime(username);
            console.log(playtimeMS);
            let playtime;
            yield formatDuration(playtimeMS)
                .then((formattedTime) => {
        ...
        yield logMessage('Sending Profile Page...', 'info');
    }
    catch (err) {
...
function getPlaytime(username) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const [user] = yield queryDb(`SELECT id FROM users WHERE username = ?`, [username]);
        console.log(user);
        // If the user doesn't exist, return 0 playtime.
        if (!user)
            return 0;
        // Get the playtime row from the database.
        const row = (yield queryDb(`SELECT playtime FROM playtime WHERE user_id = ?`, [user.id]));
        console.log(row);
...
app.post('/post/playtime', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
...
function quizlet(id) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield logMessage('Getting Quizlet data...', 'info');
...
function getQuizletDetails(id) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const response = yield fetch(`https://quizlet.com/webapi/3.4/sets/${id}`, {
            headers: {
...
app.get('/get/quizlet/list', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logMessage('Getting Quizlet list...', 'info');
        const rows = yield queryDb('SELECT id, quizlet_id, quizlet_title, quizlet_def_language, quizlet_term_language FROM quizlet', []);
        console.log(rows);
        res.json(rows);
    }
    catch (error) {
 ...
 */
const logMessage = (message, level) => __awaiter(void 0, void 0, void 0, function* () {
    // Get the stack trace to find the caller's file and line number.
    const stack = yield StackTrace.get();
