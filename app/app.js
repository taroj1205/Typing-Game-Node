...
app.post('/login', async (req, res) => {
	...
	try {
		...
		const {
			authToken,
			expiration_date
		} = await generateAuthToken(db, username);
...
app.get('/get/quizlet/data', async (req, res) => {
	try {
		...
		const {
			...
			termLang,
			defLang
		} = await getQuizletDetails(quizlet_id);
...
app.get('/leaderboard', async (req, res) => {
	...
	try {
		if (req.query.quizlet_id === undefined) {
			...
			const rows = await queryDb(db, 'SELECT u.id AS user_id, u.username, COUNT(h.id) AS word_count FROM users u LEFT JOIN history h ON u.id = h.user_id AND h.quizlet_id = ? GROUP BY u.id HAVING word_count > 0 ORDER BY word_count DESC', [quizlet_id]);
			console.log(rows);
